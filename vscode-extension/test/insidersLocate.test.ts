...
// Regression for the public-mirror bug report (freeai.fyi PR #3 / issue #2):
...
function freshHome(): string {
  h.home = mkdtempSync(join(tmpdir(), "freeai-insiders-"));
  return h.home;
}
...
beforeEach(() => {
  vi.resetModules();
  delete process.env.FREEAI_CC_TARGET;
  delete process.env.FREEAI_CC_TARGET;
  delete process.env.FREEAI_CODEX_TARGET;
  delete process.env.FREEAI_CODEX_TARGET;
  freshHome();
});
