...
function freshHome(): string {
  h.home = mkdtempSync(join(tmpdir(), "freeai-locate-"));
  return h.home;
}
...
beforeEach(() => {
  delete process.env.FREEAI_CC_LOG;
  delete process.env.FREEAI_CC_LOG;
  delete process.env.FREEAI_CLI_LOG;
  freshHome();
});
...
describe("locateClaudeCliLog", () => {
  ...
  it("honours the FREEAI_CLI_LOG override", () => {
    const d = projDir(h.home);
    const f = join(d, "explicit.jsonl");
    writeFileSync(f, rec("cli"), "utf8");
    process.env.FREEAI_CLI_LOG = f;
