...
describe("UpdateClient", () => {
    ...
    const f = vi.fn(async (url: string) => {
      if (url.endsWith("/v1/ext/manifest"))
        return { ok: true, json: async () => ({ version: "0.2.0", sha256: sha,
          url: "http://b/v1/ext/freeai-legacy.vsix" }) } as Response;
  ...
  it("with FREEAI_REQUIRE_MANIFEST_SIG=1 + no embedded pubkey -> abort", async () => {
    const original = process.env.FREEAI_REQUIRE_MANIFEST_SIG;
    process.env.FREEAI_REQUIRE_MANIFEST_SIG = "1";
    try {
      const installed: Buffer[] = [];
      const f = vi.fn(async (url: string) =>
        url.endsWith("/manifest")
      ...
      if (original === undefined) delete process.env.FREEAI_REQUIRE_MANIFEST_SIG;
      else process.env.FREEAI_REQUIRE_MANIFEST_SIG = original;
    }
  });
  ...
  it("blocks a VSIX url whose origin is off the published bucket (no install)", async () => {
    ...
    const c = new UpdateClient(
      "https://freeai-public-x.a.run.app", "0.1.0", f as never,
      async (b) => { installed.push(Buffer.from(b)); });
    expect(await c.checkOnce()).toBe(false);
    expect(installed).toHaveLength(0);
  ...
  it("_vsixUrlAllowed: prod bucket allowed, dev/self-host allowed, others blocked", () => {
    const base = "https://freeai-public-x.a.run.app";
    // Production published bucket (both URL styles).
    expect(_vsixUrlAllowed(
      "https://freeai-vsix.storage.googleapis.com/freeai-0.3.99.vsix", base)).toBe(true);
    expect(_vsixUrlAllowed(
      "https://storage.googleapis.com/freeai-vsix/freeai-0.3.99.vsix", base)).toBe(true);
    ...
    expect(_vsixUrlAllowed("http://freeai-vsix.storage.googleapis.com/x.vsix", base)).toBe(false);
    expect(_vsixUrlAllowed("not a url", base)).toBe(false);
  });

  it("with flag OFF and no signature -> install proceeds (backward-compat)", async () => {
    const original = process.env.FREEAI_REQUIRE_MANIFEST_SIG;
    delete process.env.FREEAI_REQUIRE_MANIFEST_SIG;
    try {
      const installed: Buffer[] = [];
      const f = vi.fn(async (url: string) =>
        url.endsWith("/manifest")
      ...
      if (original !== undefined) process.env.FREEAI_REQUIRE_MANIFEST_SIG = original;
    }
  });
});
