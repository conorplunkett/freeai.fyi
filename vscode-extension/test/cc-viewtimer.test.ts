...
describe("CC view-timer billing fixes (audit #8/#15/#23)", () => {
  it("#8: a failed dock (composer miss) ENDS the overlay session — no "
    ...
    expect(h.doc.querySelector('[data-freeai-overlay="1"]')).toBeTruthy();
    ...
    h.advance(2_000);
    await sleep(500);
    expect(h.doc.querySelector('[data-freeai-overlay="1"]')).toBeNull();
  ...
  it("#15: the banner session ENDS while hidden during a turn "
    ...
    expect(h.doc.querySelector('[data-freeai-banner="1"]')).toBeTruthy();
