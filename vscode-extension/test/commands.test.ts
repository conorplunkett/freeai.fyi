 ...
 *  redirect is non-optional: AuthClient writes ~/.freeai/auth.json, and
...
describe("contributed commands → registered handlers", () => {
  const CONTRIBUTED = [
    ...
    "freeai.debugMenu",
    "freeai.editConfig",
    "freeai-legacy.signIn",
    "freeai-legacy.signOut",
    "freeai-legacy.restore",
    "freeai-legacy.status",
    "freeai-legacy.debugMenu",
  ];
  ...
  it("every legacy freeai-legacy.* alias points to the SAME closure as its freeai.* twin", async () => {
    const t = await boot();
    try {
      ...
      const pairs: [string, string][] = [
        ["freeai.signIn",    "freeai-legacy.signIn"],
        ["freeai.signOut",   "freeai-legacy.signOut"],
        ["freeai.restore",   "freeai-legacy.restore"],
        ["freeai.status",    "freeai-legacy.status"],
        ["freeai.debugMenu", "freeai-legacy.debugMenu"],
      ];
...
// ---------------------------------------------------------------------------
// freeai.signIn → opens the broker URL in the system browser AND
...
// ---------------------------------------------------------------------------
describe("freeai.signIn", () => {
  it("opens the broker URL via vscode.env.openExternal", async () => {
    const t = await boot();
    try {
      await commands.executeCommand("freeai.signIn");
      ...
      expect(await t.ctx.secrets.get("freeai.access")).toBe("AT-INT");
    } finally { await t.dispose(); }
  });

  it("the legacy freeai-legacy.signIn alias also opens the broker URL", async () => {
    const t = await boot();
    try {
      await commands.executeCommand("freeai-legacy.signIn");
      expect(_opened.some((u) => u.includes("https://broker.test/auth"))).toBe(true);
    } finally { await t.dispose(); }
  });
});
...
// ---------------------------------------------------------------------------
// freeai.signOut → clears the in-process token, restores Claude Code,
// and shows the "signed out" toast.
// ---------------------------------------------------------------------------
describe("freeai.signOut", () => {
  it("clears the access token, calls adapter.restore, and toasts", async () => {
    ...
    try {
      ...
      await commands.executeCommand("freeai.signOut");

      expect(await t.ctx.secrets.get("freeai.access")).toBeUndefined();
...
// ---------------------------------------------------------------------------
describe("freeai.signOut → ad-rotation clear", () => {
...
// ---------------------------------------------------------------------------
// freeai.restore → byte-exact revert of the patched binary(s).
// ---------------------------------------------------------------------------
describe("freeai.restore", () => {
  it("calls adapter.restore() exactly once", async () => {
    const t = await boot();
    try {
      t.adapter.restore.mockClear();
      await commands.executeCommand("freeai.restore");
...
describe("freeai.status", () => {
  it("shows an info toast that reflects signed-out state", async () => {
    ...
    try {
      ...
      expect(last!.text).toMatch(/freeai/i);
    } finally { await t.dispose(); }
  });
...
// ---------------------------------------------------------------------------
// freeai.debugMenu → opens the QuickPick. We assert the menu actually
...
describe("freeai.debugMenu", () => {
  it("opens a QuickPick containing every documented menu row", async () => {
    ...
    try {
      ...
      await commands.executeCommand("freeai.debugMenu");
...
// ---------------------------------------------------------------------------
// freeai.editConfig → materialises ~/.freeai/config.json if missing
...
// ---------------------------------------------------------------------------
describe("freeai.editConfig", () => {
  it("opens the config file in the editor", async () => {
    const t = await boot();
    try {
      _openedDocs.length = 0;
      await commands.executeCommand("freeai.editConfig");
      // ensureConfigFile() resolves to either ~/.freeai/config.json or
      // ~/.freeai/config.json depending on which exists; match either.
