...
export function locateClaudeCode(): string | null {
  ...
  const explicit = process.env.FREEAI_CC_TARGET
    || process.env.FREEAI_CC_TARGET;
...
// none are tagged (older CC builds). FREEAI_CC_LOG overrides. "" => LogTail
// yields null => block self-simulates.
export function locateClaudeCodeLog(): string {
  const explicit = process.env.FREEAI_CC_LOG
    || process.env.FREEAI_CC_LOG;
  if (explicit && existsSync(explicit)) return explicit;
  try {
...
export function locateClaudeCliLog(): string {
  const explicit = process.env.FREEAI_CLI_LOG;
  if (explicit && existsSync(explicit)) return explicit;
  try {
    const cands = scanTranscripts();
    let newestUntagged = "";
