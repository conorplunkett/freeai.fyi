...
//    When FREEAI_REQUIRE_MANIFEST_SIG=1 and a public key is compiled in
//    via FREEAI_MANIFEST_PUBKEY_PEM (esbuild --define), the manifest
...
// download origin to the published `freeai-vsix` GCS bucket in production,
...
export function _vsixUrlAllowed(rawUrl: string, base: string): boolean {
  ...
  // Production: HTTPS to the published freeai-vsix GCS bucket only.
  if (u.protocol !== "https:") return false;
  if (host === "freeai-vsix.storage.googleapis.com") return true;
  if (host === "freeai-vsix.storage.cloud.google.com") return true;
  if ((host === "storage.googleapis.com" || host === "storage.cloud.google.com")
      && u.pathname.startsWith("/freeai-vsix/")) return true;
  return false;
}
...
export class UpdateClient {
   ...
   *  so the user sees "FreeAI: v0.3.85 available, installing…" while
