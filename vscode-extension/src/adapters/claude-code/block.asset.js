...
(function () {
  "use strict";
  var TIER = __FREEAI_TIER__;
  var AD = __FREEAI_AD__;
  var ICON_REF = __FREEAI_ICON__;
  var ICON_URL = __FREEAI_ICON_URL__;
  var PORT = __FREEAI_PORT__;
  var LBTOKEN = __FREEAI_LBTOKEN__;
  var CLICKTOKEN = __FREEAI_CLICKTOKEN__;
  ...
  var CLICKURL = __FREEAI_CLICKURL__;
  ...
  var BANNER_ON = __FREEAI_BANNER_ON__;
  ...
  var BASE = __FREEAI_BASE__ || ("http://127.0.0.1:" + PORT + "/freeai/" + LBTOKEN);
  ...
  function buildAdHtml(tier, s) {
    ...
    var A1 = '<a href="' + href + '" target="_blank" ' +
      'rel="noopener noreferrer" data-freeai-ad="1" style="color:';
  ...
  // creative. data-freeai-ad="1" reuses the existing capture-phase click
  ...
  function buildBannerHtml(ad, clickUrl) {
    ...
    return '<span style="display:inline-flex;align-items:center;gap:6px;' +
      ...
      'data-freeai-ad="1" style="color:' + FG +
  ...
  try {
    ...
    var DEBUG = __FREEAI_DEBUG__;
    var _seq = 0, _dlogFails = 0;
    // Relay a timestamped lifecycle line to the loopback (→ server-side
    // ~/.freeai/debug.log) so a headless agent can diagnose without a
    ...
    // (baked into the block as __FREEAI_VIEW_THRESHOLD_MS__; falls back
    // to 15_000 ms when the server didn't specify). Pure best-effort:
    // any throw is swallowed (prime directive).
    var THRESHOLD_MS = (typeof __FREEAI_VIEW_THRESHOLD_MS__ === "number"
      && __FREEAI_VIEW_THRESHOLD_MS__ > 0)
    ...
    document.addEventListener("click", function (ev) {
      ...
      while (el && el !== document) {
        if (el.getAttribute && el.getAttribute("data-freeai-ad")) {
          ...
          while (p && p !== document) {
            try {
              if (p.getAttribute && p.getAttribute("data-freeai-banner") === "1") {
                surface = "banner"; break;
              }
              if (p.getAttribute && p.getAttribute("data-freeai-overlay") === "1") {
    ...
    if (BANNER_ON) setInterval(function () {
      try {
        ...
        if (el.getAttribute("data-freeai-banner") === "1" &&
            _bannerLast === html) {
        ...
        el.innerHTML = html;
        el.setAttribute("data-freeai-banner", "1");
    ...
    function ensureOverlay(row) {
      ...
      overlay.setAttribute("data-freeai", String(TIER));
      overlay.setAttribute("data-freeai-overlay", "1");
    ...
    function retargetDockedOverlay() {
      try {
        var a = overlay && overlay.querySelector('a[data-freeai-ad]');
