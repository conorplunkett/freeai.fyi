(function () {
  ...
  try {
    if (window.__freeAiCodexBoot) return undefined;   // one bootstrap / webview
    window.__freeAiCodexBoot = 1;
  ...
  try {
    ...
    var CORR = __FREEAI_CORR__, DEBUG = __FREEAI_DEBUG__;
    var PORT = __FREEAI_PORT__, LBTOKEN = __FREEAI_LBTOKEN__;
    var BASE = __FREEAI_BASE__ ||
      ("http://127.0.0.1:" + PORT + "/freeai/" + LBTOKEN);
    var GRACE_MS = 1500;
    ...
    // into the block as __FREEAI_VIEW_THRESHOLD_MS__ (fallback 15s). Pure
    ...
    function buildAd(dots, elapsed) {
      ...
      // probe like `[data-freeai-ad] svg` finds it — the row 08 / 09
      ...
      var A = '<a href="' + href + '" target="_blank" rel="noopener noreferrer" ' +
        'data-freeai-ad="1" style="display:inline-flex;align-items:center;' +
        'gap:7px;color:' + FG +
    ...
    function ensureOverlay(row) {
      ...
      overlay.setAttribute("data-freeai", "codex");
    ...
    // The baked __FREEAI_AD__ used to be FROZEN for the life of the webview:
    ...
    try {
      var D = __FREEAI_DEBUG__, C = __FREEAI_CORR__,
        B = __FREEAI_BASE__ || ("http://127.0.0.1:" + __FREEAI_PORT__ +
          "/freeai/" + __FREEAI_LBTOKEN__);
      if (D) fetch(B + "/log", { method: "POST", keepalive: true,
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ evt: "codex.throw", corr: C, t: "codex",
          msg: String(__freeaierr && __freeaierr.message || __freeaierr)
