// FreeAI CLI status line. Shipped raw (placeholders substituted at install).
...
try {
  const CACHE = __FREEAI_CLI_AD_PATH__;
  const FRESH_MS = __FREEAI_FRESH_MS__;
...
try {
  ...
  const PREV = __FREEAI_CLI_PREV_PATH__;
  ...
  if (cmd && !cmd.includes(__FREEAI_SCRIPT_NAME__)) {
    ...
    const CHAIN_TIMEOUT_MS = __FREEAI_CHAIN_TIMEOUT_MS__;
    const DRAIN_MS = 150;
