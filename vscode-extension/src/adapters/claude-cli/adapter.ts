...
const ABSENT = " FREEAI-ABSENT";
const SCRIPT_NAME = "freeai-statusline.mjs";
...
export class ClaudeCliStatuslineAdapter implements TargetAdapter {
   ...
   *  dir (for ~/.freeai) is its grandparent (<home>/.claude/settings.json). */
  constructor(settingsPath: string) {
    this.settings = resolve(settingsPath);
    this.home = dirname(dirname(this.settings));
  }

  private backupPath(): string { return this.settings + ".freeai-backup"; }
   ...
   *  (cleared ~/.freeai, AV tooling, disk error) so the user's entry is
  ...
  private renderScript(): string {
    ...
    return tpl
      .split("__FREEAI_CLI_AD_PATH__").join(JSON.stringify(this.cachePath()))
      .split("__FREEAI_CLI_PREV_PATH__").join(JSON.stringify(this.prevPath()))
      .split("__FREEAI_FRESH_MS__").join(String(FRESH_MS))
      .split("__FREEAI_SCRIPT_NAME__").join(JSON.stringify(SCRIPT_NAME))
      .split("__FREEAI_CHAIN_TIMEOUT_MS__").join(String(CHAIN_TIMEOUT_MS));
  }
