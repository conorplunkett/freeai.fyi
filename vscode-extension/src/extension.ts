...
const BASE = (() => {
  ...
  if (v.startsWith("http://")) {
    ...
    if (!looplike) {
      ...
      console.error(`FreeAI: refusing non-loopback HTTP base "${v}". ` +
        `Set FREEAI_BASE (or ~/.freeai/config.json) to https://...`);
...
const UPDATE_BASE = resolveUpdateBase(CFG, process.env.FREEAI_UPDATE_BASE);
...
const KILL_CONFIRMED_KEY = "freeai.kill.confirmed";
...
export async function activate(ctx: vscode.ExtensionContext): Promise<void> {
  try {
    ...
    const adapter: TargetAdapter = override?.adapter ??
      new ClaudeCodeAdapter(target || "/__freeai_no_target__");
    ...
    // FreeAI" / Restore command reads disabled here and sticks; the
    ...
    ctx.subscriptions.push(
      vscode.commands.registerCommand("freeai.debugMenu", dm),
      vscode.commands.registerCommand("freeai-legacy.debugMenu", dm),
      vscode.commands.registerCommand("freeai.editConfig", ec),
    ...
    if (testHooksEnabled()) {
      ctx.subscriptions.push(
        vscode.commands.registerCommand("freeai.test.disableInjection",
          async () => {
        ...
        vscode.commands.registerCommand("freeai.test.enableInjection",
          async () => {
      ...
      void vscode.commands.executeCommand(
        "setContext", "freeai.test.enabled", true);
    ...
    try {
      ...
      const LAST_HANDLED_KEY = "freeai-legacy.reload.lastHandledMtimeMs";
    ...
    // template on the first "Edit FreeAI config…") just adopts the new
    ...
    if (!claudeOk && !codexOk) {
      ...
      // turns both-incompatible (e.g. ~/.freeai/codex.disabled set after a
    ...
    if (testHooksEnabled()) {
      ...
      ctx.subscriptions.push(
        vscode.commands.registerCommand("freeai.test.refreshStatusBar",
...
export async function deactivate(): Promise<void> {
  ...
  try {
    const canaryPath = join(homedir(), ".freeai", "boot.canary");
