...
export function setupEarningsRefresh(
  ...
  const showActive = async (): Promise<void> => {
    ...
    // 30s of a kill or a deliberate "Disable FreeAI". Safety/truth
    ...
    if (!gate.enabled || gate.suspended) {
      // User-disabled (or crash-canary suspended): the red "FreeAI: Off —
  ...
  if (!auth.accessToken()) {
    statusBar.set({ kind: "signed-out" });
    const NUDGE_KEY = "freeai.signinNudge.shownAt";
      ...
      void (async () => {
        try {
          const choice = await vscode.window.showInformationMessage?.(
            "FreeAI: sign in to start earning on Claude Code spinner ads.",
            "Sign in", "Later");
          if (choice === "Sign in") {
