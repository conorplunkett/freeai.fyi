...
export function interpret(d: AdapterDiagnostics): string {
  ...
  if (!d.targetExists)
    return "VERDICT: Claude Code not found at the target path. Is the Claude Code "
      + "extension installed? (No action for FreeAI.)";
  ...
  if (d.backup.exists && !d.backup.hasArray && d.live.hasArray)
    return "VERDICT: stale backup but the live file is fine — should self-heal on "
      + "the next apply. FIX: update FreeAI to the latest build and reload.";
...
export function interpretCodex(
  ...
  if (p.optOut)
    return "VERDICT: Codex ad-serving is explicitly disabled "
      + "(~/.freeai/codex.disabled or FREEAI_CODEX=0).";
  if (!p.discoveryEnabled)
    return "VERDICT: Codex detected but Codex ad-serving is OFF — it is "
      ...
      + "To serve ads in Codex too: set FREEAI_CODEX=1 or create "
      + "~/.freeai/codex.enabled, then reload.";
  return compatible
...
/** Register the `FreeAI: Diagnose` command (+ legacy alias). Opens the report
  ...
  const run = async (): Promise<void> => {
    ...
    try {
      await vscode.window.showInformationMessage?.(
        "FreeAI diagnostics copied to clipboard.");
    } catch { /* best-effort */ }
  };
  return [
    vscode.commands.registerCommand("freeai.diagnose", run),
    vscode.commands.registerCommand("freeai-legacy.diagnose", run),
  ];
}
