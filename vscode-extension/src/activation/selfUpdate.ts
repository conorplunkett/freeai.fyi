...
const UPD_KEY = "freeai-legacy.update.attempted";
const UPD_TRANSIENT_KEY = "freeai-legacy.update.transient";
...
const UPD_INSTALLED_KEY = "freeai-legacy.update.installed";
...
export function setupSelfUpdate(
  ...
  const installVsix = async (vsix: ArrayBuffer): Promise<void> => {
    const p = join(tmpdir(), `freeai-update-${Date.now()}.vsix`);
    ...
    // ran "Disable FreeAI"; undefined/true means default-on or already-on.
    // The old unconditional `= true` stomped an explicit opt-out on every
    // self-update, so the only durable opt-out was uninstall.
    if (ctx.globalState.get<boolean>("freeai.debug.on") !== false) {
    ...
    void (async () => {
      try {
        const choice = await vscode.window.showInformationMessage?.(
          "FreeAI updated. Reload window to activate the new build?",
  ...
  const onUpdateAvailable = (info: { version: string; current: string;
    ...
    try {
      const msg = info.rollback
        ? `FreeAI: rolling back to v${info.version} (from v${info.current})…`
        : `FreeAI: v${info.version} available — installing now…`;
  ...
  const updater = new UpdateClient(updateBase, currentVersion,
    ...
    recordLkg: (v, vsix) => {
      try {
        const lkgPath = join(tmpdir(), `freeai-lkg-${v}.vsix`);
        writeFileSync(lkgPath, vsix);
        void ctx.globalState.update("freeai-legacy.update.lkg", { v, path: lkgPath });
      } catch { /* best-effort */ }
    },
  }, onUpdateAvailable);
