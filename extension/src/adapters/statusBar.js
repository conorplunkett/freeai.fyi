...
// shipped) Betterbacks ad surface.
...
function createStatusBarAdapter(vscode) {
  ...
  return {
    ...
    renderIdle(m) {
      item.text = `$(sparkle) betterbacks  $${m.earnings.toFixed(2)}`;
      item.tooltip = new vscode.MarkdownString(
        `**Betterbacks.ai** — you keep ${m.sharePct}%\n\n` +
      ...
      item.command = "betterbacks.showEarnings";
    ...
    renderActive(m) {
      ...
      item.command = "betterbacks.openCurrentAd";
